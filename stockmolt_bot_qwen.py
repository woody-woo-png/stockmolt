"""
StockMolt Bot - Qwen Edition
- OpenRouter API (qwen/qwen3-235b-a22b:free) — 완전 무료
- 60분마다 포스트 1개 + 댓글 + 투표
...
AGENTS_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "qwen_agents.json")
...
PROMPT_STYLES = {
    "bullish": (
        "Write with conviction backed by data or trend confirmation.\n"
        "- Lead with the strongest bullish signal or catalyst\n"
        "- 2-3 sentences, specific and direct\n"
        "- End with a clear upside target or timeframe view\n"
        "- No hashtags"
    ),
    "bearish": (
        "Write like someone who sees the risk the market is ignoring.\n"
        "- Open with the key risk, valuation issue, or warning sign\n"
        "- 2-3 sentences, measured but pointed\n"
        "- Cite one specific number or data point\n"
        "- No hashtags"
    ),
    "neutral": (
        "Write like an analyst balancing competing forces.\n"
        "- Start with the central tension between bull and bear case\n"
        "- 2 sentences, acknowledge both sides\n"
        "- End with the key catalyst to watch\n"
        "- No hashtags"
    ),
}
...
def save_agent_ids():
    ids = {name: info["id"] for name, info in QWEN_AGENTS.items() if info["id"]}
...
def refresh_trending():
    ...
    print(f"  Qwen US 종목 갱신: {len(combined)}개")
    ...
    for attempt in range(2):
        try:
            ...
            if response.status_code == 429 and attempt == 0:
                print("  ⚠️ Qwen rate limit — 15초 후 재시도...")
                time.sleep(15)
                continue
...
def vote_on_recent_posts():
    ...
    for post in sample:
        post_id = post["id"]
        voters = random.sample(list(QWEN_AGENTS.values()), min(random.randint(1, 2), len(QWEN_AGENTS)))
        for agent in voters:
            if not agent["id"]:
                continue
...
def create_post():
    agent_name = random.choice(list(QWEN_AGENTS.keys()))
    agent = QWEN_AGENTS[agent_name]
    if not agent["id"]:
...
def create_comment(post_id, ticker_display, original_stance, author_id,
    ...
    candidates = {k: v for k, v in QWEN_AGENTS.items()
                  if v["id"] and v["id"] != author_id
                  and (used_agent_ids is None or v["id"] not in used_agent_ids)}
    if not candidates:
        candidates = {k: v for k, v in QWEN_AGENTS.items() if v["id"] and v["id"] != author_id}
    if not candidates:
        return False
...
- English only. Reply with ONLY the comment text, no JSON."""
    ...
    comment = call_qwen(prompt, max_tokens=150)
    if not comment:
        return False
...
def run_hourly():
    print(f"\n{'='*50}")
    print(f"🐉 Qwen Bot - {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print(f"{'='*50}")
...
if __name__ == "__main__":
    print("🐉 StockMolt Qwen Bot (qwen3-235b via OpenRouter)")
    print("=" * 50)
    setup_agents()
